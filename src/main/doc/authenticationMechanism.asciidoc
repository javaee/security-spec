...
** If _isNewAuthentication()_ returns true, update the request state to record that it is a caller-initiated authentication, and continue with flow _processCallerInitiatedAuthentication_.
** If _isNewAuthentication()_ returns false, continue with flow _processContainerInitiatedAuthentication_.
...
* Redirect or forward to the _LoginToContinue.loginPage()_ depending on the value of the _useForwardToLogin()_ attribute.
...
** If the _LoginToContinue.errorPage()_ is non-null and non-empty, redirect to _LoginToContinue.errorPage()_.
* Return the _AuthenticationStatus_.
...
* Call the _HttpMessageContext.notifyContainerAboutLogin()_ method with the _CallerPrincipal_ and groups from the saved authentication state.
...
The _AutoApplySession_ annotation provides the ability to declaratively add _javax.servlet.http.registerSession_ behavior to an authentication mechanism and auto-apply it for every request.

The _javax.servlet.http.registerSession_ behavior is described in Section 3.8.4 of [https://jcp.org/aboutJava/communityprocess/mrel/jsr196/index2.html[JASPIC]].

This annotation defines the concept of a caller being authenticated over a series of multiple HTTP requests (together, a "session"). The form-based authentication mechanisms use this same concept. Therefor it is implicitely suggested, but not required, that such form-based authentication mechanisms use the backing interceptor for this annotation to establish and maintain their session.
...
** Obtain the _CallbackHandler_ from _HttpMessageContext_, and let it handle the _CallerPrincipalCallback_.
** Return _AuthenticationStatus.SUCCESS_.
...
Any _LoginToContinue_-annotated _HttpAuthenticationMechanism_, including the two built-in FORM authentication mechanisms, can be triggered via a call to the _SecurityContext.authenticate()_ method. This method is based on the _HttpServletRequest.authenticate()_ method, as defined by [https://jcp.org/aboutJava/communityprocess/final/jsr340/index.html[SERVLET31]], but has been extended to support additional functionality defined by the Servlet Container Profile of [https://jcp.org/aboutJava/communityprocess/mrel/jsr196/index2.html[JASPIC]].

The extended behavior is facilitated by the _AuthenticationParameters_ parameter passed to _SecurityContext.authenticate()_. _AuthenticationParameters_ includes a _ewAuthentication_ field.

When _newAuthentication_ is set to _true_, the container MUST discard any state that it holds for a _LoginToContinue_-annotated _HttpAuthenticationMechanism_ and that is associated with the current caller. Specifically, this means that any associated state, such as described for the <<LoginToContinue Annotation>> above, MUST be cleared, and the request must proceed as if processing a new request.
