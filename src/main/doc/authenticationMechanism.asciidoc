...
The _AutoApplySession_ annotation provides an application the ability to declaratively add "javax.servlet.http.registerSession" functionality to an authentication mechanism and auto applies this for every request.

"javax.servlet.http.registerSession" refers to the requirements as descrived in the JASPIC 1.1 specification section 3.8.4.
...
The container MUST provide an interceptor implementation at priority _PLATFORM_BEFORE + 200_ that backs the _AutoApplySession_ annotation and intercepts calls to the configured _HttpAuthenticationMechanism_. The interceptor MUST behave as follows when intercepting calls to the _HttpAuthenticationMechanism_:

Intercepting _validateRequest()_::
* Obtain the _Principal_ from the _HttpServletRequest_ that is obtained from the _HttpMessageContext_ passed in as argument to the _validateRequest()_ method. For sake of discussion call this principal _principal_ and the _HttpMessageContext_ _httpMessageContext_.
* If _principal_ is null:
** Call the next Interceptor. For the sake of discussion call its outcome _outcome_.
** If _outcome_ equals _SUCCESS_:
*** Obtain the _map_ object of the _messageInfo_ from _httpMessageContext_. For sake of discussion call this _map_.
*** Add an entry to _map_ with key "javax.servlet.http.registerSession" and value "true".
** Return _outcome_.
* Otherwise
** Create a new _CallerPrincipalCallback_ instance with as constructor arguments the client subject obtained from _httpMessageContext_, and _principal_. For the sake of discussion call this instance _callerPrincipalCallback_.
** Obtain the _CallbackHandler_ from _httpMessageContext_ and let it handle _callerPrincipalCallback_.
** Return _SUCCESS_.
